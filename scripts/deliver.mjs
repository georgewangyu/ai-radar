...
const userDir = path.join(homedir(), ".ai-radar");
...
async function readConfig() {
  try {
    ...
    if (error.code === "ENOENT") return { delivery: { method: "stdout" } };
    throw error;
  }
}
...
async function sendEmail(text, apiKey, to) {
  const response = await fetch("https://api.resend.com/emails", {
    body: JSON.stringify({
      from: "AI Radar <digest@resend.dev>",
...
async function main() {
  ...
  if (method === "telegram") {
    if (!env.TELEGRAM_BOT_TOKEN) throw new Error("TELEGRAM_BOT_TOKEN not found in ~/.ai-radar/.env");
    if (!config.delivery?.chatId) throw new Error("delivery.chatId not found in ~/.ai-radar/config.json");
    await sendTelegram(text, env.TELEGRAM_BOT_TOKEN, config.delivery.chatId);
  } else if (method === "email") {
    if (!env.RESEND_API_KEY) throw new Error("RESEND_API_KEY not found in ~/.ai-radar/.env");
    if (!config.delivery?.email) throw new Error("delivery.email not found in ~/.ai-radar/config.json");
